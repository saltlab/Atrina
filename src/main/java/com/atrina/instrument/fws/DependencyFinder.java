...
import com.atrina.instrument.helpers.ControlMapper;
import com.atrina.instrument.helpers.FunctionCallParser;
import com.atrina.instrument.helpers.InfixExpressionParser;
import com.atrina.instrument.helpers.NotParser;
import com.atrina.instrument.helpers.ObjectLiteralParser;
import com.atrina.instrument.helpers.PropertyGetParser;
import com.atrina.units.FunctionArgumentPair;
...
public class DependencyFinder extends AstInstrumenter {
	...
	private void handleAssignmentOperator(InfixExpression node) {
		...
		if (rightSide.getType() == org.mozilla.javascript.Token.GETPROP) {
			...
			varBeingRead = "this";
			isInterestingRead=varBeingRead.equals(variableName);
	...
	private boolean isInteresting(ArrayList<AstNode> varDeps){
		...
		while (it.hasNext()) {
            operand = it.next();
			if (operand instanceof Name && ((Name) operand).getIdentifier().equals(variableName)) {
				found = true;
				break;
			} else if (operand instanceof KeywordLiteral && operand.toSource().equals("this") && "this".equals(this.variableName)) {
				found = true;
				break;
			} 
        }
	...
	private boolean isAnArgument(FunctionCall node) {
		...
		while (argumentIterator.hasNext()) {
			...
			if (nextArgument instanceof Name && ((Name) nextArgument).getIdentifier().equals(variableName)) {
				found = true;
				break;
			} else if (nextArgument instanceof KeywordLiteral && nextArgument.toSource().equals("this") && "this".equals(variableName)) {
				found = true;
				break;
			}
		}
