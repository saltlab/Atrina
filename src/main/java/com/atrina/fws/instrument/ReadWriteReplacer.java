...
import com.atrina.configuration.TraceHelper;
import com.atrina.instrument.AstInstrumenter;
import com.atrina.instrument.helpers.FunctionCallParser;
import com.atrina.instrument.helpers.InfixExpressionParser;
import com.atrina.instrument.helpers.PropertyGetParser;
import com.atrina.units.SlicingCriteria;
...
public class ReadWriteReplacer extends AstInstrumenter {
	...
	private void handleVariableDeclaration(VariableDeclaration node) {
		...
		while (varIt.hasNext()) {
			...
			if (rightSide.getType() == org.mozilla.javascript.Token.NAME && isItInteresting(((Name) rightSide).getIdentifier(), rightSide.getLineno())) {// ((Name) leftSide).getIdentifier().equals(variableName)) {
				...
				newBody = (VARWRITE+"(\""+leftSide.toSource()+"\", " +rightSide.toSource()+ " , "+ newAlias +", "+node.getLineno()+", \""+this.getScopeName()+"\")");
					 ...
					 && !((FunctionCall) rightSide).getTarget().toSource().contains(TOOLNAME)
					 && isAnArgument((FunctionCall) rightSide)){
	...
	private void handleAssignmentOperator(Assignment node) {
		...
		String varBeingRead = "";
		boolean isInterestingRead=false;
		...
		if (rightSide.getType() == org.mozilla.javascript.Token.GETPROP) {
			...
			isInterestingRead=isItInteresting(varBeingRead, rightSide.getLineno());
			...
			varBeingRead = "this";
			isInterestingRead=isItInteresting(varBeingRead, rightSide.getLineno());
		} else if(rightSide.getType() == org.mozilla.javascript.Token.SUB ||
				rightSide.getType() == org.mozilla.javascript.Token.ADD){
			ArrayList<AstNode> operands=InfixExpressionParser.getOperandDependencies((InfixExpression)rightSide, true);
			isInterestingRead=isInteresting(operands);
		} else if(rightSide.getType() == org.mozilla.javascript.Token.CALL){
		
			isInterestingRead=isAnArgument((FunctionCall)rightSide);
		} else if(node.getOperator() == org.mozilla.javascript.Token.ASSIGN
				&& isItInteresting(varBeingRead, rightSide.getLineno())){
			isInterestingRead=true;
		} else if(rightSide.getType() == org.mozilla.javascript.Token.GETPROP){
			ArrayList<AstNode> operands=PropertyGetParser.getPropertyDependencies((PropertyGet)rightSide);
			isInterestingRead=isInteresting(operands);
		}
		...
		if (isInterestingRead){// || isItInteresting(varBeingWritten, leftSide.getLineno()) ){ 
	...
	private boolean isItInteresting (ArrayList<String> varNames, ArrayList<Integer> lineNos) {
		if (varNames.size() == 0 || varNames == null) {
			System.out.println("[isItInteresting]: Invalid variable name.");
			return false;
		}
		ArrayList<Scope> scopes=new ArrayList<Scope>();
		for(int i=0;i<varNames.size();i++){
			scopes.add(TraceHelper.getDefiningScope(astCrawled, varNames.get(i), lineNos.get(i)));
		}

		ArrayList<SlicingCriteria> checkMes=new ArrayList<SlicingCriteria>();
		for(int i=0;i<varNames.size();i++)
			checkMes.add(new SlicingCriteria(scopes.get(i), varNames.get(i), true));

		Iterator<SlicingCriteria> it = this.variablesOfInterest.iterator();
		Iterator<SlicingCriteria> checkMeIt=checkMes.iterator();
		SlicingCriteria next;
		SlicingCriteria nextCheckMe;
		while (it.hasNext()) {
			next = it.next();
			while(checkMeIt.hasNext()){
				nextCheckMe=checkMeIt.next();
				if (nextCheckMe.equals(next)) {
					System.out.println("[isItIteresting]: Returning true");
					return true;
				}
			}
	...
	private boolean isInteresting(ArrayList<AstNode> varDeps){
		
		Iterator<AstNode> it = varDeps.iterator();
		AstNode operand;
        boolean found=false;
		while (it.hasNext()) {
            operand = it.next();
			if (operand instanceof Name && isItInteresting(((Name) operand).getIdentifier(), operand.getLineno())) {
				found = true;
				break;
			} else if (operand instanceof KeywordLiteral && operand.toSource().equals("this") && isItInteresting("this", operand.getLineno())) {
				found = true;
				break;
			} 
        }
